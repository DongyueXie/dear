...
s.factor = estimateSizeFactorsForMatrix(thout$mat)
vout = limma::voom(counts = thout$mat%*%diag(1/s.factor), design = new_design)
lout = limma::lmFit(vout)
...
s.factor = estimateSizeFactorsForMatrix(Y)
vout = limma::voom(counts = Y%*%diag(1/s.factor), design = new_design)
lout = limma::lmFit(vout)
#lout = limma::lmFit(log(Y%*%diag(c(1/colSums(Y)))*1e6),design = new_design)
eout2 = eBayes(lout)
